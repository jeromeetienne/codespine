---
description: Autofix one queued GitHub issue — branch off main, implement the smallest fix, conflict-check it against every other open autofix PR, gate on typecheck + test, then open a PR and label the issue 'autofixed'. Never merges.
...
## Step 5 — Gate on typecheck + test

With no conflict, run the checks locally:
...
- **Fail** → this fix is not good enough to ship. Label the issue
  ...
  gh issue comment <number> --body "Autofix attempt failed \`npm run typecheck && npm test\`:
...
Checked locally with \`npm run typecheck && npm test\` — both pass."
```
...
- Never open a PR whose `npm run typecheck && npm test` did not pass locally.
