...
const BASE: GraphReportData = {
	...
	keyFindings: [
		{ title: 'Highest blast radius', detail: 'the largest reaches 2 symbol(s), 67% of the codebase — change these carefully.', tone: 'risk', symbols: [{ name: 'helper', kind: 'Function', filePath: 'src/a.ts', startLine: 3 }] },
		{ title: 'Dead code is safe to remove', detail: '1 exported symbol(s) across 1 file(s) are never referenced — confirm, then delete.', tone: 'opportunity', symbols: [{ name: 'unusedThing', kind: 'Function', filePath: 'src/b.ts', startLine: 9 }] },
	],
};
...
describe('GraphReport.render markdown', () => {
	it('includes the headline, banner, and every section heading', () => {
		...
		for (const heading of ['## Key findings', '## Snapshot', '## Composition', '## Load-bearing code', '## Communities', '## Runtime', '## Cycles', '## System boundary', '## Cleanup candidates', '## Where to go next']) {
	...
	it('footers a link to the tool repository, not the analysed repo', () => {
		const markdown = GraphReport.render(BASE, 'markdown');
		assert.ok(markdown.includes('Generated by [codespine](https://github.com/jeromeetienne/codespine)'));
		assert.equal(markdown.includes('codespine report'), false);
	});
});

describe('GraphReport key findings', () => {
	it('renders each finding as a bullet with github-linked subject symbols', () => {
		const markdown = GraphReport.render(BASE, 'markdown');
		assert.ok(markdown.includes('## Key findings'));
		assert.match(markdown, /- \*\*Highest blast radius:\*\* \[`helper`\]\(https:\/\/github\.com\/acme\/widget\/blob\/abcdef1234567890\/src\/src\/a\.ts#L3\) — /);
		assert.match(markdown, /- \*\*Dead code is safe to remove:\*\* \[`unusedThing`\].* — /);
	});

	it('shows a graceful line when there are no findings', () => {
		const markdown = GraphReport.render({ ...BASE, keyFindings: [] }, 'markdown');
		assert.match(markdown, /## Key findings\n\n_Nothing stands out/);
	});

	it('renders findings as tone-coloured cards in the visual html', () => {
		const html = GraphReport.renderVisualHtml(BASE);
		assert.ok(html.includes('<h2>Key findings</h2>'));
		assert.ok(html.includes('class="finding risk"'));
		assert.ok(html.includes('class="finding opportunity"'));
		assert.ok(html.includes('Highest blast radius'));
	});
});
