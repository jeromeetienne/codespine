...
instantiation, and value reads. Broader than [`who-calls`](/commands/who-calls), which
only follows `CALLS`.
...
| --- | --- | --- |
| `<id>` | yes | Node id of the symbol or type to inspect. Obtain it from [`find --json`](/commands/find). |
...
[`dead-exports`](/commands/dead-exports) uses to decide whether a symbol is unused.
...
| Question | Command |
| --- | --- |
| Who *calls* this (CALLS only)? | [`who-calls`](/commands/who-calls) |
| Who references this *in any way* (8 reference kinds)? | `references` |
| Every edge touching this, both directions, all kinds? | [`neighbors`](/commands/neighbors) |
...
- Empty results may also mean a stale id — re-run [`find`](/commands/find) to confirm.

## See also

- [`who-calls`](/commands/who-calls) — the narrower, calls-only view.
- [`dead-exports`](/commands/dead-exports) — applies this reference set across all exports.
- [`neighbors`](/commands/neighbors) — all edges, both directions.
