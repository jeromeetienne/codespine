...
| --- | --- | --- |
| `<id>` | yes | Node id of the symbol to analyse. Obtain it from [`find --json`](/commands/find). |
...
Conceptually this is [`who-calls`](/commands/who-calls) applied repeatedly: the direct
...
- The traversal follows `CALLS` only. Type-level impact (a changed type rippling
  ...
  [`references`](/commands/references) for the broader, one-hop reference set.
...
- [`who-calls`](/commands/who-calls) — the one-hop version (`depth 1`).
- [`references`](/commands/references) — broader edge kinds, one hop.
