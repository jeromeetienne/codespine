...
and after an edit — and report the delta. Where [`verify`](/commands/verify) is the
...
same [`enrich`](/commands/enrich) path, and the metric is read back through the
[`cost`](/commands/cost) model. The one new idea is **honesty about noise** — runtime
...
| --- | --- |
| `<target>` | The symbol to benchmark, by **name** (e.g. `titleCase`). It is resolved against the *current* graph like [`find`](/commands/find), so it survives the line shifts an edit causes — you do not pass a line-bound node id. |
...
| `-d, --db <path>` | `./outputs/graph.kuzu` | Kùzu database to read the target and its cost from. |
...
| `--baseline <file>` | — | Compare the current median against the median saved in this file, and report the delta (advisory). |
| `--save-baseline <file>` | — | Write this run as a baseline for a later `--baseline` comparison. |
...
bundled example, [`scripts/benchmarks/project_01_workload.ts`](https://github.com/jeromeetienne/ts_knowledge_graph/blob/main/scripts/benchmarks/project_01_workload.ts),
...
#     --db ./outputs/project_01/graph.kuzu --root ./sample_projects/project_01 --runs 5
```

## A before/after measurement

The metric is read against the *current* graph, so after an edit you **rebuild
the graph** (the edit shifts line numbers) before measuring again. Because
`projectNN:rebuild` wipes `./outputs/<project>`, keep the baseline file
**outside** that directory:

```bash
# 1. Baseline, before the edit (save it somewhere the rebuild won't wipe).
...
npx ts-knowledge-graph benchmark titleCase --workload scripts/benchmarks/project_01_workload.ts \
  ...
  --save-baseline ./outputs/bench/p01_titleCase.baseline.json
...
npx ts-knowledge-graph benchmark titleCase --workload scripts/benchmarks/project_01_workload.ts \
  --db ./outputs/project_01/graph.kuzu --root ./sample_projects/project_01 \
  --baseline ./outputs/bench/p01_titleCase.baseline.json
```
...
It type-checks and still passes project_01's tests, so [`verify`](/commands/verify)
...
- [`verify`](/commands/verify) — the hard correctness gate. Run it first: keep an edit only if it still type-checks and passes tests, *then* ask `benchmark` whether it helped.
- [`enrich`](/commands/enrich) / [`cost`](/commands/cost) — the runtime-ingest and cost-attribution machinery `benchmark` reuses to read a node's measured time.
- [`/code-graph-optimize`](/agent/slash-commands) — the agent that applies an edit, verifies it, and can report its measured impact.
