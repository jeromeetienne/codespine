...
[guide d'analyse statique](/static-analysis) : celui-ci répond à des questions *structurelles*
à la main ; celui-ci boucle de la *mesure* vers une *modification
vérifiée*.

La même boucle pilote l'agent [`/code-graph-optimize`](/agent/slash-commands).
...
La commande en lecture seule [`/code-graph-interview`](/agent/slash-commands) transforme une
...
Changez une seule chose — à la main, ou laissez [`/code-graph-optimize`](/agent/slash-commands)
...
- [`hotspots`](/commands/hotspots) · [`cost`](/commands/cost) ·
  ...
  boucle enchaîne.
- [Agent](/agent/slash-commands) — `/code-graph-optimize` exécute cette boucle
  de façon autonome ; `/code-graph-interview` cadre la cible.
- [Analyse statique](/static-analysis) — le pendant structurel, piloté à la main.
- [Parcourir le graphe](/explore) — orientez-vous avant d'optimiser.
