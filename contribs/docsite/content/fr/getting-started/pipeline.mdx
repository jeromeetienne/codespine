...
3. **query / optimize** — les commandes de parcours répondent aux questions d'analyse d'impact ;
   la commande Claude Code `/codespine-optimize` confie ces mêmes requêtes à un
...
Cela écrit la base de données Kùzu embarquée dans `./.codespine/graph.kuzu` — dérivée de
`-o, --output-folder` (valeur par défaut `./.codespine`), la même base que toutes les autres commandes
lisent.
...
`/codespine-optimize` (voir [Agent](/agent/slash-commands)). Il n'y a aucun
...
projet, installez-les avec `npx codespine install` (voir
...
Une commande compagne en lecture seule, `/codespine-interview`, vous interroge pour cadrer
une cible d'optimisation mesurable et ancre chaque candidat dans le graphe,
produisant des tâches que vous pouvez ensuite confier à `/codespine-optimize`.

## Dépannage

| Symptôme | Cause / correctif |
|---|---|
| `/codespine-optimize` n'est pas une commande connue | Les commandes ne sont pas installées dans `.claude/`. Exécutez `npx codespine install` (ou, dans ce dépôt, `npm run symlink:dotclaude`). |
...
| Erreurs Kùzu au sujet du répertoire de la base de données | Un autre processus maintient peut-être la base ouverte, ou la base provient d'une version de Kùzu incompatible. Exécutez `rm -rf .codespine/graph.kuzu` et rechargez. |
