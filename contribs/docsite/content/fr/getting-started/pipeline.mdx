...
|---|---|
| `/codespine-optimize` n'est pas une commande connue | Les commandes ne sont pas installées dans `.claude/`. Exécutez `npx ts-knowledge-graph install` (ou, dans ce dépôt, `npm run symlink:dotclaude`). |
