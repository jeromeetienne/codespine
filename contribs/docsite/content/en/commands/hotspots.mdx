...
counterpart to the raw traversal queries: where [`who-calls`](/commands/who-calls) and
[`blast-radius`](/commands/blast-radius) answer questions about one node, `hotspots`
...
by [`enrich`](/commands/enrich). The static metrics (`callers`, `call-count`,
...
- **Runtime metrics need [`enrich`](/commands/enrich).** Without it the graph has no
...
- **`blast-radius` here is unbounded**, unlike the [`blast-radius`](/commands/blast-radius)
...
- **Static call edges only.** Dynamic dispatch is invisible, so a real hotspot
  ...
  [`who-calls`](/commands/who-calls) / [`blast-radius`](/commands/blast-radius) before acting.
- **Re-extract before trusting it.** The loader merges by id and does not remove
  ...
  see [`load`](/commands/load).

## See also

- [`enrich`](/commands/enrich) — attach the `metadata.runtime` the runtime metrics rank by.
- [`blast-radius`](/commands/blast-radius) — the depth-bounded, single-node version of the transitive-reach metric.
- [`dead-exports`](/commands/dead-exports) — the other opinionated detector: the safest edits (removable code) rather than the highest-leverage ones.
- [`/codespine-interview`](/agent/slash-commands) — scopes an optimization target; consumes this ranking instead of re-deriving it by hand.
