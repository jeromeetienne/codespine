...
- [`/code-graph-optimize`](/agent/slash-commands) — the agent that calls `verify` to keep or revert each edit.
- [`benchmark`](/commands/benchmark) — the *measured-impact* gate: did the edit actually make the targeted metric better? (Advisory, distinct from this hard pass/fail gate.)
