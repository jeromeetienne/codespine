...
| Symptom | Cause / fix |
|---|---|
| `/codespine-optimize` is not a known command | The commands are not installed into `.claude/`. Run `npx ts-knowledge-graph install` (or, inside this repo, `npm run symlink:dotclaude`). |
