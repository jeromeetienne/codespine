...
```bash
npm run build    # embed ../../.codespine/graph/*.jsonl into web/js_autogenerated/graph_data.js
...
`scripts/build-data.ts` reads `../../.codespine/graph/{nodes,edges}.jsonl` by
...
`.codespine/graph/nodes.jsonl` + `.codespine/graph/edges.jsonl` onto it.

**C. Serve the repo root** — the page then auto-fetches
`../../../.codespine/graph/*.jsonl`:
...
When the graph has been enriched (`codespine enrich <profile.cpuprofile>`),
