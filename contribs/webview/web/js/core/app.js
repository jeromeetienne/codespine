	...
	static async tryFetch() {
		try {
			const [nodesText, edgesText] = await Promise.all([
				fetch('../../../.codespine/graph/nodes.jsonl').then((r) => r.ok === true ? r.text() : Promise.reject(new Error(String(r.status)))),
				fetch('../../../.codespine/graph/edges.jsonl').then((r) => r.ok === true ? r.text() : Promise.reject(new Error(String(r.status)))),
			]);
			Graph.setData(DataLoading.parseJsonl(nodesText), DataLoading.parseJsonl(edgesText), 'fetched ../../../.codespine/graph/*.jsonl');
		} catch {
