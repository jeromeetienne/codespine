...
| `-o, --output-folder <dir>` | `./outputs` | Output folder; reads the database from `<dir>/graph.kuzu` and writes baselines under `<dir>/bench/`. |
...
#     -o ./outputs/project_01 --root ./sample_projects/project_01 --runs 5
```
...
npx ts-knowledge-graph benchmark titleCase --workload scripts/benchmarks/project_01_workload.ts \
  -o ./outputs/project_01 --root ./sample_projects/project_01 --baseline
```
