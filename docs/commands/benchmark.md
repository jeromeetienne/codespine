...
| `-o, --output-folder <dir>` | `./.codespine` | Output folder; reads the database from `<dir>/graph.kuzu` and writes baselines under `<dir>/bench/`. |
...
`projectNN:rebuild` wipes `./.codespine/<project>` but **preserves** its
